...
def get_home_html(today_count=0):
    ...
    @media (prefers-color-scheme: dark) {{
        .banner {{ background-color: #2D2D2D; color: #eee; }}
        .square-btn {{ background: #2D2D2D; color: #eee; }}
        .material-symbols-rounded {{ color: #eee; }}
        #qr-code-img {{ filter: brightness(0.8); }}
        .banner div[style*="color: #666"] {{ color: #aaa !important; }}
    }}
    </style>
...
def get_cached_styles():
    ...
    @media (prefers-color-scheme: dark) {{
        ...
        [data-testid="stForm"]::before, [data-testid="stAppViewBlockContainer"]:has(#home-bg-marker)::before {{ 
    ...
    @media (prefers-color-scheme: dark) {{
        ...
        .order-card.odd-card, .order-card.even-card {{ background: transparent; }}
        .order-card {{ border: none; color: #eee; }}
        .order-card-wrapper:not(:last-child) .order-card {{ border-bottom: 1px solid #333; }}
        .summary-item, .summary-total, .receipt-date {{ color: #eee; }}
        .summary-box-wrapper {{ filter: none; }}
        .summary-box {{
            background: #2D2D2D;
            box-shadow: none;
        }}
        ...
        .updated-icon {{ color: #7B9FFF; }}
        .first-icon {{ color: #FFB74D; }}
    }}
    ...
    .updated-icon {{ font-family: 'Material Symbols Rounded'; font-size: 0.85rem; color: #245EFF; font-variation-settings: 'wght' 400, 'FILL' 1, 'opsz' 20; vertical-align: middle; line-height: 1; }}
    .first-icon {{ font-family: 'Material Symbols Rounded'; font-size: 0.85rem; color: #245EFF; font-variation-settings: 'wght' 400, 'FILL' 0, 'opsz' 20; vertical-align: middle; line-height: 1; }}
    ...
    button[data-variant="pills"] p, button[data-variant="pills"]:hover p, button[data-variant="pills"]:focus p {{ color: #333 !important; }}
    button[data-variant="pills"][aria-checked="true"], button[data-variant="pills"][aria-pressed="true"], button[data-variant="pills"][aria-checked="true"]:hover, button[data-variant="pills"][aria-pressed="true"]:hover {{
        ...
        border: 1px solid #FCEFE2 !important;
        color: #8B4318 !important;
        min-height: 37px !important;
    }}
    ...
    .stTextInput div[data-testid="stTextInputRootElement"]:focus-within {{ background-color: #F3F4F6 !important; border-color: transparent !important; outline: none !important; box-shadow: none !important; }}
    ...
    /* 토스트 커스텀 스타일 */
    [data-testid="stToast"] {{
        background-color: rgba(253, 243, 209, 0.7) !important;
...
def show_orders():
    ...
    for _, row in today_sorted.iterrows():
        ...
        updated_badge = '<span class="updated-icon">verified</span>' if row.get('수정여부', False) else ''
        first_badge = '<span class="first-icon">thumb_up</span>' if row['연번'] == 1 else ''
            ...
            <div class="order-card-wrapper">
                <div class="order-card {bg_class}">
                    ...
                    <div class="card-right">
                        <div class="order-no">{first_badge}{updated_badge}ORDER #{row["연번"]:02d}</div>
...
def show_order_modal():
    ...
    if submitted:
        if not name.strip() or not menu.strip() or not temp:
            st.toast("이름, 메뉴, 음료 온도는 필수 항목입니다.", icon=":material/warning:")
        else:
